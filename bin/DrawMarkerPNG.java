...
public class DrawMarkerPNG {
    static void drawArc(Graphics2D g, double x, double y, double a0, double a1, double r0, double r1) {
	final int N = 100;
	GeneralPath gp = new GeneralPath();

	gp.moveTo(x + Math.cos(a0)*r0, y + Math.sin(a0)*r0);

	for(int i=1;i<=N;i++) {
	    gp.lineTo(x + Math.cos(a0 + (a1-a0)*i/N)*r0, y + Math.sin(a0 + (a1-a0)*i/N)*r0);
	}

	gp.lineTo(x + Math.cos(a1)*r1, y + Math.sin(a1)*r1);

	for(int i=1;i<=N;i++) {
	    gp.lineTo(x + Math.cos(a1 + (a0-a1)*i/N)*r1, y + Math.sin(a1 + (a0-a1)*i/N)*r1);
	}
	...
	if (args.length == 0) {
	    System.out.println("Usage : java DrawMarkerPNG <data> [<magninifation>]");
	    System.out.println("This program draws a marker corresponding to data (a number between 0 and 500)");
	    System.out.println("in PNG format, and write it to output.html.");
	    System.exit(-1);
	}

	System.setProperty("java.awt.headless", "true");

	final int cs = 1024, bs = 512;

	BufferedImage image = new BufferedImage(cs, cs, BufferedImage.TYPE_INT_RGB);
	...
	int data = 0;
	if (args.length != 0) data = Integer.parseInt(args[0]);
	int cw;
	cw = data >= 0 ? code[data] : Integer.parseInt("01010101010101010101010", 2);

	double mag = 1.0;
	if (args.length >= 2) mag = Double.parseDouble(args[1]);

	int PL = 19; // physical/bit length
	int HPL = PL/2 + 1;
	double BW = 131 * mag;

	drawArc(g, cs/2, cs/2, -Math.PI*+0.0, -Math.PI*+2.0, Math.sqrt(HPL+0) * BW, Math.sqrt(HPL+1) * BW);
	drawArc(g, cs/2, cs/2, -Math.PI*+0.0, -Math.PI*+2.0, Math.sqrt(HPL+2) * BW, Math.sqrt(HPL+3) * BW);

	for(int i=0;i<HPL;i++) {
	    if (((cw >> (PL-i-1)) & 1) != 0) {
		drawArc(g, cs/2, cs/2, -Math.PI*+0.0, -Math.PI*+1.0, Math.sqrt(HPL - i) * BW, Math.sqrt(HPL - i - 1) * BW);
	    }
	}

	for(int i=0;i<HPL;i++) {
	    if (((cw >> i) & 1) != 0) {
		drawArc(g, cs/2, cs/2, -Math.PI*+1.0, -Math.PI*+2.0, Math.sqrt(HPL - i) * BW, Math.sqrt(HPL - i - 1) * BW);
	    }
	}
